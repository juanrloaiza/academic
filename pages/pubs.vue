...
<template>
  ...
  <Article v-for="(pub, index) in forthcoming" :pub="pub" :key="`pub-${index}-f`" class="mb-4 md:mb-2" />

  <Article v-for="(pub, index) in pubArticles" :pub="pub" :key="`pub-${index}-p`" class="mb-4 md:mb-2" />

  <h3 class="text-2xl font-bold mt-6 mb-4 md:mb-2">{{ $t('bookchapters') }}</h3>
  <BookChapter v-for="(pub, index) in pubChapters" :pub="pub" :key="`pub-${index}-f`" class="mb-4 md:mb-2" />

  <h3 class="text-2xl font-bold mt-6 mb-4 md:mb-2">{{ $t('monographs') }}</h3>
  <Book v-for="(pub, index) in pubBooks" :pub="pub" :key="`pub-${index}-f`" class="mb-4 md:mb-2" />
